    ...
    render: ({ form }) => {
        ...
        return (
            <div className="flex flex-col gap-2">
                    ...
                    {fieldInteractions.title && (
                        ...
                        >
                            <form.AppField
                                ...
                                listeners={{
                                    onChange: () => {
                                        if (!fieldInteractions.description) {
                                            setFieldInteractions((prev) => ({
                                                ...prev,
                                                description: true,
                                            }))
                ...
                {/* Visibility Field - Appears after description */}
                <AnimatePresence>
                    {fieldInteractions.description && (
                        <motion.div
                            initial={{ opacity: 0, height: 0, y: -20 }}
                            animate={{ opacity: 1, height: 'auto', y: 0 }}
                            exit={{ opacity: 0, height: 0, y: -20 }}
                        ...
                        >
                            <form.AppField
                                ...
                                listeners={{
                                    onChange: ({ value }: { value: 'private' | 'shared' | 'public' }) =>
                                        setFieldInteractions((prev) => ({
                                            ...prev,
                                            visibility: value,
                                        })),
                ...
                <AnimatePresence>
                    {fieldInteractions.visibility === 'private' && (
                        <motion.div
                            initial={{ opacity: 0, height: 0, y: -20 }}
                            animate={{ opacity: 1, height: 'auto', y: 0 }}
                            exit={{ opacity: 0, height: 0, y: -20 }}
                                            ...
                                            <field.EmailField
                                                ...
                                                onKeyDown={(e: React.KeyboardEvent) => {
                                                    if (e.key === 'Enter') {
                                                        ...
                                                        const email =
                                                            form.state.values.currentInviteEmail
                                            ...
                                            {field.state.value.length > 0 && (
                                                <div className="mt-4 space-y-2">
                                                    ...
                                                    <div className="space-y-2 max-h-40 overflow-y-auto">
                                                        <AnimatePresence>
                                                            {field.state.value.map((invite: InvitationInput) => (
                                                                <motion.div
                                                                    key={invite.id}
                                                                        ...
                                                                        onClick={() => {
                                                                            field.handleChange(
                                                                                field.state.value.filter(
                                                                                    (inv: InvitationInput) => inv.id !== invite.id,
                                                                                ),
                                                                            )
                                                                        }}
                                                                    >
