        ...
        >
          <div className="flex w-full items-center justify-between py-2.5 pl-5 pr-2.5">
            ...
            <div className="relative">
              ...
              >
                <X strokeWidth={4} className={`size-4 ${t.closeBtnIcon}`} />
              </button>
            </div>
          </div>
          <div
            className={`flex flex-col gap-2.5 rounded-[22px] ${t.panelBg} p-2.5 shadow-[0_-3px_3px_-1.5px_rgba(0,0,0,0.08)] ring-1 ring-black/[8%] ${t.panelBgDark} dark:border dark:border-gray-800 transition-all duration-300 ${
              isHovered ? 'scale-95 opacity-95' : 'scale-100 opacity-100'
            }`}
          >
              ...
              {actions.map((item, index) => (
                ...
                >
                  <div className="flex flex-col items-center gap-2">
                    {item.icon}
                    <p className={`text-xs ${t.itemText} ${t.itemTextDark}`}>
                      {item.name}
                    </p>
                  </div>
                </button>
...
export default function CreateNewProjectBox({
  actions,
  onActionClick,
  label,
  theme,
